...
class UnitBase {
 public:
  UnitBase() = delete;
  static inline Unit_T Zero() { return Unit_T(0); }
  static inline Unit_T PlusInfinity() { return Unit_T(PlusInfinityVal()); }
  static inline Unit_T MinusInfinity() { return Unit_T(MinusInfinityVal()); }

  inline bool IsZero() const { return value_ == 0; }
  inline bool IsFinite() const { return !IsInfinite(); }
  inline bool IsInfinite() const {
    return value_ == PlusInfinityVal() || value_ == MinusInfinityVal();
  }
  inline bool IsPlusInfinity() const { return value_ == PlusInfinityVal(); }
  inline bool IsMinusInfinity() const {
    return value_ == MinusInfinityVal();
  }

  inline bool operator==(const Unit_T& other) const {
  ...
  inline Unit_T RoundTo(const Unit_T& resolution) const {
    RTC_DCHECK(IsFinite());
  ...
  inline Unit_T RoundUpTo(const Unit_T& resolution) const {
    RTC_DCHECK(IsFinite());
    RTC_DCHECK(resolution.IsFinite());
    RTC_DCHECK_GT(resolution.value_, 0);
  ...
  inline Unit_T RoundDownTo(const Unit_T& resolution) const {
    RTC_DCHECK(IsFinite());
    RTC_DCHECK(resolution.IsFinite());
    RTC_DCHECK_GT(resolution.value_, 0);
 ...
 protected:
  ...
  static inline Unit_T FromValue(T value) {
    if (Unit_T::one_sided)
  ...
  template <
      typename T,
      typename std::enable_if<std::is_integral<T>::value>::type* = nullptr>
  static inline Unit_T FromFraction(int64_t denominator, T value) {
    if (Unit_T::one_sided)
      RTC_DCHECK_GE(value, 0);
  ...
  template <typename T>
  inline T ToValueOr(T fallback_value) const {
  ...
  inline int64_t ToFractionOr(int64_t fallback_value) const {
  ...
  explicit inline UnitBase(int64_t value) : value_(value) {}

 private:
  template <class RelativeUnit_T>
  friend class RelativeUnit;

  static inline int64_t PlusInfinityVal() {
    return std::numeric_limits<int64_t>::max();
  }
  static inline int64_t MinusInfinityVal() {
    return std::numeric_limits<int64_t>::min();
  }

  inline Unit_T& AsSubClassRef() { return static_cast<Unit_T&>(*this); }
  inline const Unit_T& AsSubClassRef() const {
    return static_cast<const Unit_T&>(*this);
  }
  // Assumes that n >= 0 and d > 0.
  static inline int64_t DivRoundPositiveToNearest(int64_t n, int64_t d) {
    return (n + d / 2) / d;
  }
  // Assumes that d > 0.
  static inline int64_t DivRoundToNearest(int64_t n, int64_t d) {
...
class RelativeUnit : public UnitBase<Unit_T> {
 public:
  inline Unit_T Clamped(Unit_T min_value, Unit_T max_value) const {
    return std::max(min_value,
                    std::min(UnitBase<Unit_T>::AsSubClassRef(), max_value));
  }
  inline void Clamp(Unit_T min_value, Unit_T max_value) {
    *this = Clamped(min_value, max_value);
  }
  inline Unit_T operator+(const Unit_T other) const {
  ...
  inline Unit_T& operator+=(const Unit_T other) {
    *this = *this + other;
  ...
  inline double operator/(const Unit_T other) const {
    return UnitBase<Unit_T>::template ToValue<double>() /
           other.template ToValue<double>();
  }
  template <typename T>
  inline typename std::enable_if<std::is_arithmetic<T>::value, Unit_T>::type
  operator/(const T& scalar) const {
    return UnitBase<Unit_T>::FromValue(
        std::round(UnitBase<Unit_T>::template ToValue<int64_t>() / scalar));
  }
  inline Unit_T operator*(double scalar) const {
    return UnitBase<Unit_T>::FromValue(std::round(this->ToValue() * scalar));
  }
  inline Unit_T operator*(int64_t scalar) const {
    return UnitBase<Unit_T>::FromValue(this->ToValue() * scalar);
  }
  inline Unit_T operator*(int32_t scalar) const {
    return UnitBase<Unit_T>::FromValue(this->ToValue() * scalar);
  }
...
template <class Unit_T>
inline inline Unit_T operator*(double scalar, RelativeUnit<Unit_T> other) {
  return other * scalar;
}
template <class Unit_T>
inline  Unit_T operator*(int64_t scalar, RelativeUnit<Unit_T> other) {
  return other * scalar;
}
template <class Unit_T>
inline  Unit_T operator*(int32_t scalar, RelativeUnit<Unit_T> other) {
  return other * scalar;
}
template <class Unit_T>
inline  Unit_T operator-(RelativeUnit<Unit_T> other) {
